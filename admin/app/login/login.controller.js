...
function loginController($scope, $state, $rootScope, APIURL, $http, ApiService) {
    ...
    $scope.user = localStorage.getItem('nobeladmin');
    ...
    if($scope.current_state == 'confirm'){
        ...
        $rootScope.loggedInUserInfo = JSON.parse(localStorage.getItem('nobeladmin'));
        $state.go('blogs');
    }
    ...
    $scope.authenticate = function() {
        ApiService.login($scope.login).then(function (response) {
            if (response.status === 'Success') {
                ...
                localStorage.setItem('nobeladmin', JSON.stringify(response.data));
